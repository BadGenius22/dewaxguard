...
> **Reasoning role (per `rules/model-tiering.md`)**: spawn every Feynman and State pass with a unique task name, `fork_turns="none"`, and high reasoning effort when supported. Nemesis runs only in `thorough` mode, and its cross-feed work is recall-sensitive even though this subprocess only coordinates passes and detects convergence.

For Feynman passes, spawn a Codex subagent with this prompt:
