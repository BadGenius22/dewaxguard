...
For each file below, check existence and record a one-line summary. Keep preflight bounded to roughly 15 focused file reads.

### 1a — Audit boundaries + scope

| File | Path | What to extract |
|------|------|-----------------|
| Codex instructions | applicable `AGENTS.md` files from `{{PROJECT_ROOT}}` upward | scope boundaries, build commands, security constraints, platform rules |
| Legacy target instructions | `{{PROJECT_ROOT}}/CLAUDE.md` | additional scope/build context when present; `AGENTS.md` wins on conflict |
...
| Failure-mode ledger | `{{SKILL_ROOT}}/failure-modes/INDEX.md` | historical class-level gaps; use as weighting, not as a finding list |
| Audit evidence rules | `{{SKILL_ROOT}}/references/audit-method.md` | quiet-pass, cited-mechanism, and PoC-integrity rules |
| Threat-model references | `~/.codex/audit-references/threat-model-references.md` | methodology/examples; revalidate moving links and derive target facts independently |

Read all available rows. They prime the target-specific threat model and pre-refute dead hypotheses without substituting prior work for current evidence.
...
|------|------|--------|----------------|
| AGENTS.md | applicable path(s) | PRESENT/ABSENT | ... |
| CLAUDE.md | {{PROJECT_ROOT}}/CLAUDE.md | PRESENT/ABSENT | legacy context only |
...
- failure-modes/INDEX.md: PRESENT/ABSENT — <N> class-level gaps loaded
- references/audit-method.md: PRESENT/ABSENT — A-1/A-2/A-3 loaded
- threat-model-references.md: PRESENT/ABSENT — target-specific application recorded
...
- [ ] Applicable AGENTS.md and threat-model references were read or absence recorded
