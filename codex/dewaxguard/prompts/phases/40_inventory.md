...
> You are a fresh `codex exec --json` subprocess invoked by the dewaxguard v1.13 driver.
...
If the audit is in PROVEN_ONLY mode (set in applicable AGENTS.md, legacy `{{PROJECT_ROOT}}/CLAUDE.md`, or the invocation), append `--proven-only` to the command.
