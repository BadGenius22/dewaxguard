...
> Fresh `codex exec --json` subprocess invoked by the dewaxguard v1.13 driver.
...
Spawn 4 recon sub-agents via Codex subagent delegation. They run in parallel — do NOT chain them sequentially. Each writes its assigned output and stops.
...
- Any whitepaper or design doc cited in applicable AGENTS.md, legacy CLAUDE.md, or CONTEST_FAQ.md
- Applicable AGENTS.md files (authoritative audit instructions)
- {{PROJECT_ROOT}}/CLAUDE.md (legacy audit-specific context, if present)
...
3. **External programs**: enumerate every external program / cross-program-invocation / oracle / bridge / dependency outside the audit scope. For each, record: name, version, called from <files>, called for <purpose>, assumed behavior (the assumption your contract makes — if violated, what breaks).
   - **Mainnet program ID verification (Solana / EVM cross-check)**: Many Solana repos ship `declare_id!` PLACEHOLDER values that DO NOT match the actual mainnet deployment (dev keypairs auto-generated by `build.js` on first run). EVM repos sometimes ship the wrong address in `deployments/*.json`. Before treating audited program IDs as mainnet addresses, cross-check against the project's published SDK: grep `node_modules/@<org>/chain-solana/src/**/Chains.ts`, `node_modules/@<org>/sdk/dist/**`, or fetch the SDK via `gh api repos/<org>/<sdk-repo>/git/trees/main?recursive=1` then read `*Chains.ts` / `*Chains.d.ts`. Look for hardcoded `swapContract: "..."`, `btcRelayContract: "..."`, `programId: "..."`, `address: "0x..."`, etc. **If the SDK's mainnet ID differs from `declare_id!` / `deployments/*.json`, document BOTH** in `design_context.md` under External programs — the mainnet ID is required for any `[FORK-PASS]` verification in Phase 5c (see `rules/fork-poc-execution.md`). Skip if no mainnet deployment is claimed (testnet-only / unlaunched protocols).
...
Write to {{SCRATCHPAD}}/design_context.md with sections:
  ...
  ## Audit-specific scope (from AGENTS.md / legacy CLAUDE.md / CONTEST_FAQ.md)
