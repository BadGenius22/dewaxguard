...
>
> **Reasoning role: commander / decision gate.** Per `rules/model-tiering.md`, use the configured Codex default unless the caller supplies `--commander-model`. Platform accept/reject scoring is low-token and high-judgment. Do all scoring in this subprocess; do not spawn subagents.
...
Detect the target platform from applicable `AGENTS.md`, legacy `{{PROJECT_ROOT}}/CLAUDE.md`, or `{{SCRATCHPAD}}/CONTEST_FAQ.md`. If none names a platform, score against a neutral severity model and list platform-specific differences; do not pretend every platform simultaneously applies.
...
If applicable AGENTS.md or legacy CLAUDE.md declares a `Realism filter`, use the highest-precedence applicable instruction.
...
- out-of-scope per AGENTS.md, legacy CLAUDE.md, or contest scope: OUT_OF_SCOPE
