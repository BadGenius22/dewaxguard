...
| FM-10 | truncating division whose quotient is consumed as a price/rate/scale lands on a small NONZERO integer — 25–100% relative quantization error at ordinary honest inputs, invisible to every `!= 0` guard and to every operand-extreme probe | evm / cross-lang | RC-METHOD in 4 compounding layers: (a) `MATH_PAT` never matched `mulDiv`/`Math.*`, so the dominant modern fixed-point primitive never entered `math-map.md` and the numeric specialist never reached the file; (b) all truncation coverage was truncate-to-**zero** — nothing covered truncate-to-small-integer; (c) the trust-boundary agent that did reach it carried only operand-extreme questions, producing a divergence framing that manufactured a false admin dependency; (d) the realism filter then terminally rejected that mis-framing and chain analysis subsumed the survivor into an admin-gated hypothesis | `MATH_PAT` + soroban `mul_div`/`fixed_*` in build_recon_maps.sh; math-map header quotient-range mandate; math-precision "Quantize a live quotient"; numerical-gap Seam 2 output-domain enumeration; boundary-agent Step 6 handoff; realism-filter Gate R-0; chain-analysis Subsumption direction | 1 | 2026-08-05 (Sherlock 1279) |
