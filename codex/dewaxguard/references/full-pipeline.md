# Full DewaxGuard Pipeline Reference

This is the detailed upstream workflow adapted for Codex. The concise `SKILL.md` is authoritative for Codex invocation, paths, permissions, and model roles when this reference contains older terminology.
...
5b. `~/.agents/skills/dewaxguard/references/audit-method.md` — the three tool-independent rules that bind every audit session: **A-1** a quiet pass is not coverage, **A-2** no uncited mechanism claim crosses a boundary, **A-3** a test that cannot fail proves nothing. Read it explicitly in driver mode or any subprocess with fresh context.
5c. `~/.codex/audit-references/threat-model-references.md` — methodology and public examples. Revalidate moving sources and derive target-specific facts from the repository, deployment, and current primary evidence.

**Step 2 — Cross-audit context** (skip if file does not exist; load from `~/.agents/skills/dewaxguard/` or equivalent):
...
- [ ] `~/.agents/skills/dewaxguard/references/audit-method.md` rules A-1/A-2/A-3 are in context (auto-loaded or read explicitly)
...
> **Driver mode (v1.13+, opt-in)**: `python3 ~/.agents/skills/dewaxguard/scripts/dewaxguard_driver.py --mode core --src ./contracts [--audit-id X] [--resume]` — replaces the prompt-only LLM orchestrator with a deterministic Python driver that spawns `codex exec --json` subprocesses per phase, runs content+coverage gates between phases, and checkpoints for crash resume. Legacy prompt-only mode remains the default.
...
|------|--------|----------|
| **Light** | ~16 (all Worker) | Recon → TestGap → Model → Breadth(4) → Depth(4) → Chain → Verify → Report |
...
> `~/.agents/skills/dewaxguard/references/audit-method.md`: a quiet pass is not coverage.
...
## PHASE 2.5: FINDINGS-BLIND PROTOCOL MODEL (core/thorough; worker in light)
...
Spawn a small `mechanical` agent with the ambiguous pair list and ask it to mark each pair MERGE/SEPARATE. Update `findings_merged.json` accordingly. This is the only LLM step in Phase 4a — the rest is deterministic. Typical audit produces 0-5 ambiguous pairs.
...
| Flag | Effect |
|------|--------|
| `light` | Worker-only, no Nemesis, no fork PoC |
| `core` | Finding+Worker, fork PoC for Medium+, no Nemesis |
