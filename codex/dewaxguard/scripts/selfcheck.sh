...
echo "== 1. Version sync =="
v_file=$(tr -d ' \n' < VERSION)
v_ref=$(grep -oE '\*\*v[0-9]+\.[0-9]+\.[0-9]+\*\*' references/full-pipeline.md | head -1 | tr -d '*v')
[ "$v_file" = "$v_ref" ] || fail "VERSION ($v_file) != full-pipeline reference banner ($v_ref)"
[ $FAIL -eq 0 ] && ok "upstream v$v_file recorded in VERSION and full-pipeline reference"
...
for p in scripts/*.py scripts/gates/*.py scripts/squeezers/*.py; do
  [ -f "$p" ] && { python3 -c 'import ast, pathlib, sys; ast.parse(pathlib.Path(sys.argv[1]).read_text(encoding="utf-8"), filename=sys.argv[1])' "$p" 2>/dev/null || { fail "python syntax: $p"; sc_fail=1; }; }
done
