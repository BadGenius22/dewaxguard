...
Execution flow is a language-agnostic concept but entry points and primitives vary. For the detected language, see `~/.agents/skills/dewaxguard/prompts/{LANGUAGE}/phase4b-runtime-templates.md` section A (transaction validation phase ordering) and F (cross-tx composition).
