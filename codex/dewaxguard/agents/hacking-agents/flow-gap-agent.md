...
> **Roster**: thorough-mode attacker-framing agent (added v1.19.0, ported from solidity-auditor v3). Runs alongside the 8 core hacking agents. Cross-references the execution-trace, periphery, and first-principles agents.
...
The seam method is language-agnostic. Map "periphery call" to the detected language:
- **EVM/Solidity**: token transfer / oracle read / callback / low-level call mid-flow.
- **Solana/Rust**: CPI mid-instruction, sysvar read, account re-entry across instructions in one tx.
- **Move (Aptos/Sui)**: cross-module call, object/coin movement mid-function, hot-potato resolution path.
- **C/C++ ledger (rippled, Bitcoin Core)**: cross-transactor composition in one ledger close, helper that mutates an SLE another transactor trusts, amendment-gated branch whose end-state breaks a conservation guarantee.
