...
> **Roster**: thorough-mode attacker-framing agent (added v1.19.0, ported from solidity-auditor v3). Runs alongside the 8 core hacking agents.
...
A "boundary" is any point where control or data crosses into code/values you don't control. Examples below are Solidity; map them to the detected language:
- **EVM/Solidity**: `.call/.delegatecall/.staticcall`, `payable` + `msg.value`, sentinel-address branches, `abi.decode` of `bytes`, external return values.
- **Solana/Rust**: CPI call sites, account-data deserialization (`try_from_slice`), `remaining_accounts`, sysvar reads, lamport transfers.
- **Move (Aptos/Sui)**: calls into other modules, `BCS::from_bytes` decoding, coin/object transfers, type-argument boundaries.
- **C/C++ ledger (rippled, Bitcoin Core)**: `STTx`/`SLE` field access (`isFieldPresent`), serializer/deserializer pairs, external library calls (OpenSSL/secp256k1), amount casts, `view().peek()` returns. The "no code at receiver" case maps to "field absent / SLE null".
...
## Step 2 — For every external call: corner cases

For each call site identified in Step 1, ask:

1. **No code at receiver.** What if `address.code.length == 0`? Low-level `.call(...)` returns `(true, "")`. `IERC20(addr).approve(...)` reverts. `safeTransfer(addr, ...)` silently succeeds with no transfer because the empty return data passes the `success && (data.length == 0 || decoded == true)` check. (C++ ledger: the matching trap is an absent field / null SLE treated as a valid zero.)
...
For each `payable` function (native-value entry-point in any language):
...
## Step 5 — For every bytes input / decode: corruption cases

For every `bytes` input or `abi.decode` (or `try_from_slice` / `BCS::from_bytes` / serializer in the detected language):
