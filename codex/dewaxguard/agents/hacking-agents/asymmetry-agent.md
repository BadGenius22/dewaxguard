...
The asymmetry lens is language-agnostic — the examples below are Solidity, but apply the same write/read/branch diff to the detected language:
- **EVM/Solidity**: paired external/public functions, `if/else` native-vs-ERC20 branches, storage-slot writes.
- **Solana/Rust**: paired instructions (init ↔ close, deposit ↔ withdraw), account-mutation symmetry, `process_*` branches.
- **Move (Aptos/Sui)**: paired entry functions, resource acquire/return symmetry, `acquires` write sets.
- **C/C++ ledger (rippled, Bitcoin Core)**: paired transactors (Create ↔ Delete, Deposit ↔ Withdraw), `doApply` branches, SLE field writes, `adjustOwnerCount` create/destroy symmetry.

For C++ ledger codebases, the highest-value asymmetry is a create-site that adjusts owner count / reserves where the matching destroy-site forgets (or vice versa).
