...
>
> **🚨 REQUIRED READING**: Before starting, the agent must read `~/.claude/skills/dewaxguard/platform-quirks/stellar.md`. The entry #1 (State Archival ≠ Deletion) corrects a misconception that has historically produced invalid findings. Every runtime analysis must internalize this before tracing TTL-related attack paths.
