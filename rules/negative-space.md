...
> **Binds**: global rule A-1 (`~/.claude/audit-method.md`) — a quiet pass is not coverage.
...
**Spawn ONE agent (opus in core/thorough, sonnet in light) with a hard input restriction:**
...
3. **Value-outflow map** — every site where value leaves the system, and for each, the actor gate
   ...
   differentiated-work question from `~/.claude/bounty-calibration.md` rule 4: enumerate the
