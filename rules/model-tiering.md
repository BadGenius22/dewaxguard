...
|------|-------|-------|-----|
| **worker** | `sonnet` / `haiku` | verify (PoC + code trace), preflight, bake, inventory, and every fan-out **dispatcher** (recon, breadth, niche, depth, nemesis, chain, report) | High-token, low-judgment, or pure coordination. The dispatcher's own model does not touch finding quality — the sub-agents it spawns carry that. |
| **finding** | `opus` (core/thorough) | the breadth / depth / niche / nemesis / chain-synthesis / recon-deep **sub-agents**, plus the report Critical+High writer | Where bugs are actually found or the flagship deliverable is written. Recall-sensitive — **never down-tiered.** |
...
**Key mechanism.** A phase's `claude -p` subprocess model (set in the driver's `PHASES` registry via `Phase.model`) is **independent** of the `model=` a phase prompt passes to a `Task` sub-agent. So a `sonnet` dispatcher can spawn `opus` finding agents. That independence is what lets us make the coordination cheap without touching recall. **Every dispatcher must therefore state the sub-agent tier explicitly** — via a `model=` literal in the agent block (breadth/depth/report) or a prose model directive in the dispatch section (recon, chain, niche, nemesis). A dispatcher whose sub-agents carry no tier would silently inherit the cheap dispatcher model and down-tier the finding work.
...
- **Finding sub-agent tier** — the `model=` literals or prose model directives inside the phase prompts (`30_breadth.md` agent table, `45_depth*.md`, `47_chain.md`, `60_report.md` writer agents, `10_recon.md`, `42_niche.md` dispatch section, `46_nemesis.md` per-pass instructions). **These are the recall-critical knobs — leave them at `opus` for core/thorough.**
